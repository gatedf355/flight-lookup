...
export default function FlightLookup() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const airlineCodeMatch = q.match(/^([A-Z]{2,3})/)
      if (airlineCodeMatch) {
        ...
        if (airline && airline.icao) {
          ...
          const flightNumber = q.substring(airlineCode.length)
          // Convert to ICAO callsign format (e.g., "BA178" -> "BAW178")
          searchCallsign = `${airline.icao}${flightNumber}`
          console.log(`Converting IATA code "${airlineCode}" to ICAO callsign "${searchCallsign}" (original: "${q}")`)
        }
      }
