...
import { normalizeFlight, asFlightLevel, fmt } from "@/app/lib/normalizeFlight"
...
export default function FlightLookup() {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty("--color-accent", scheme.accent)
                  ...
                  if (cleanQuery !== searchQuery.trim().toUpperCase()) {
                    setError("Flight numbers can only contain letters and numbers (A-Z, 0-9). Please remove spaces and special characters.")
                    return
                  }
    ...
    try {
      const response = await fetch(`/api/flight?number=${encodeURIComponent(cleanQuery)}&full=true`)
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-2xl mx-auto mb-12">
              ...
              <p className="text-lg text-muted-foreground">
                Enter an airline code (e.g., UAL, DAL, AS) to get real-time tracking data
              </p>
              <p className="text-sm text-muted-foreground mt-2">
                Letters and numbers (A-Z, 0-9) are allowed - letters will be automatically converted to uppercase
              </p>
            </div>
            ...
            <CardContent className="p-6">
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  ...
                  <Input
                    placeholder="Enter airline code (e.g., UAL, DAL, AS...)"
                    value={searchQuery}
                    onChange={(e) => {
                      // Allow letters and numbers, convert letters to uppercase, filter out spaces and symbols
                      const value = e.target.value.replace(/[^A-Za-z0-9]/g, '').toUpperCase()
                      setSearchQuery(value)
                    }}
                    onKeyPress={handleKeyPress}
                    className={`pl-10 h-12 text-lg border-border focus:ring-2 focus:ring-[var(--color-primary)]/50 ${
                      searchQuery && /^[A-Z0-9]+$/.test(searchQuery) 
                        ? 'border-green-500' 
                        : searchQuery 
                        ? 'border-red-500' 
                        : ''
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {["UAL", "DAL", "AS", "THY"].map((airline) => (
                  <Button
                    key={airline}
                    variant="outline"
                    size="sm"
                    onClick={() => setSearchQuery(airline)}
                    className="text-xs"
                  >
                    {airline}
                  </Button>
        ...
        {flightData && flightData.success && (() => {
          const f = normalizeFlight(flightData);
          return (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-4 text-muted-foreground">
                      <span>FR24 ID: {f.fr24Id || 'N/A'}</span>
                      <Separator orientation="vertical" className="h-4" />
                      <span>Status: {f.status || 'N/A'}</span>
                    </div>
                  </div>
                  <Badge
                    variant="secondary"
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-4">
                            <div className="flex justify-between">
                              ...
                              <span className="font-mono text-sm">
                                {fmt.coord(f.lat)}, {fmt.coord(f.lon)}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-sm text-muted-foreground">Altitude</span>
                              <span className="font-semibold text-[var(--color-primary)]">
                                {asFlightLevel(f.altFt)}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-sm text-muted-foreground">Ground Speed</span>
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="flex justify-between">
                              <span className="text-sm text-muted-foreground">Vertical Speed</span>
                  ...
                  <CardContent>
                    <div className="text-center space-y-4">
                      <div className="p-4 bg-secondary/10 rounded-lg">
                        ...
                        <p className="font-semibold text-lg">{f.status || 'N/A'}</p>
